...
"}\n"
"// Draw ships with deck boundaries\n"
...
"ctx.fillRect(ship.deck_bounds.min_x, ship.deck_bounds.min_y, \n"
"ship.deck_bounds.max_x - ship.deck_bounds.min_x, \n"
"ship.deck_bounds.max_y - ship.deck_bounds.min_y);\n"
"// Draw bow (front)\n"
...
"const x = p.world_x; const y = p.world_y;\n"
"// Draw player circle\n"
...
"ctx.fillStyle = 'white';\n"
"ctx.font = '10px Arial';\n"
"ctx.fillText(p.name + ' (' + p.id + ')', x+6, y-6);\n"
"ctx.fillStyle = p.state === 'WALKING' ? '#2ecc71' : '#3498db';\n"
"ctx.fillText(p.state, x+6, y+4);\n"
"});\n"
"// Draw legend\n"
...
"ctx.fillStyle = 'white';\n"
"ctx.fillText('Player (on ship)', 38, 56);\n"
"ctx.fillStyle = '#3498db';\n"
"ctx.beginPath(); ctx.arc(26, 68, 4, 0, 2*Math.PI); ctx.fill();\n"
"ctx.fillStyle = 'white';\n"
"ctx.fillText('Player (swimming)', 38, 72);\n"
"}\n"
...
"const walkingCount = data.players.filter(p => p.state === 'WALKING').length;\n"
"const swimmingCount = data.players.filter(p => p.state === 'SWIMMING').length;\n"
...
"<div class=\"stat\"><span>� Walking:</span><span class=\"stat-value\">${walkingCount}</span></div>\n"
"<div class=\"stat\"><span>🏊 Swimming:</span><span class=\"stat-value\">${swimmingCount}</span></div>\n"
"`;\n"
"}\n"
